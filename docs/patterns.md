...
**Default allowlist (v8.3+).** Because `url-allowlist.txt` shipped empty, an armed session doing legitimate research (which readily arms Layer 6 — injection papers contain the scanner's trigger strings) escalated *every* research fetch, flooding the user. A small, plugin-shipped **default** allowlist (`scripts/web-safety-default-allowlist.txt`) is now layered **under** the user file via `host_in_any_list` — a match in either exempts the host. Membership is a security control: a host qualifies only if an attacker who controls a resource there cannot read back request query/path/body, so repo-analytics / user-app hosts (`github.com`, `gitlab.com`, `huggingface.co`, `readthedocs.io`) are excluded even though the raw-file CDN `githubusercontent.com` is included (finalized by DCA `20260705T195623`). Egress-guard scope only — Layer 1's pre-screen never reads it. Disable with `WEB_SAFETY_DEFAULT_ALLOWLIST_DISABLE=1`. After `WEB_SAFETY_SUGGEST_THRESHOLD` (3) asks to one host, the confirmation gains a one-shot `/web-safety:allow <host>` hint (**never** auto-adds). The guard's desktop notification is rate-limited to one per 5s. **Residual:** a redirect *from* an allowlisted host is not re-screened (the guard sees only the initial URL) — a follow-up hardening item.
